...
function overwirte(db) {
    systemUtils.updateAuthUser(
        /**
         * 读取当前登录的用户信息，然后使用当前的用户信息来做权限校验  
         * 使用箭头函数才能读取到当前环境的信息
         * @param {ctx} ctx 
        ...
        (ctx) => {
            try {
                ...
                if (error.message.includes('用户未登录!')) console.log('权限校验失败', error.message);
                else console.log('权限校验失败', error);
            }
        })
}
