...
export default function koaRouterExternalAnalysisHAR(router) {
    const 调试参数 = {
        inputFile: "./ld/plugins/externals/gen-web-by-har/input/2025-6-12 163450_PP官方_Big Bass Bonanza 1000_用于调回放的参数列表_vtgmp.prerelease-env.biz.har",
        outputDir: "./ld/plugins/externals/gen-web-by-har/output/"
    };
    router.all('外部插件-解析har文件', '/external/analysisHAR', async (ctx, next) => {
        ...
        if (!(inputFile && fs.existsSync(lif) && fs.statSync(lif).isFile())) re = '参数inputFile合并后不是一个文件的路径: ' + lif;
            ...
            re = await runCmd(`cd ./ld/plugins/externals/gen-web-by-har && npm run gen`, true, 'utf8');
        }
        ctx.body = result(re);
    });
