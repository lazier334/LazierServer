 ...
 * 生成 proxy.js 插件的demo
 */
export default createGenProxy(async function genProxyDemo(proxyFuns) {
    // 全局提示使用，在导出的时候会传递一个空对象进来，在开发环境的插件proxy.js中被更改名称为 "obj" 
    // 在生产环境被更名为 "2随机数_5随机数"
    let GlobalParam = {};

    // 给插件增加其他功能可以把函数写在这里
    if (false) createGenProxy.utils.addFunctions(proxyFuns, {
        demo:
            /**
             * demo
             */
        ...
        demo_dev: {
            ...
            run(e) {
                ...
                return createGenProxy.utils.createFunction(e.demo.toString()
